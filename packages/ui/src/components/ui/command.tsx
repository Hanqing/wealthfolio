...
const CommandDialog = ({ children, contentClassName, ...props }: DialogProps & { contentClassName?: string }) => {
  return (
    <Dialog {...props}>
      <DialogContent className={cn("shadow-modal overflow-hidden p-0 [&>button]:top-4", contentClassName)}>
...
>(({ className, ...props }, ref) => (
  <div className="flex h-14 items-center border-b px-3" data-cmdk-input-wrapper="">
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      "data-[selected='true']:bg-foreground/5 data-[selected=true]:text-foreground relative flex cursor-default select-none items-center gap-2 rounded-md px-2 py-1.5 text-sm outline-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
      className,
    )}
    {...props}
  />
